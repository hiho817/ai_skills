...
- 國家教育研究院「樂詞網」（https://terms.naer.edu.tw/）是正式術語的首要依據。查核時記錄查詢詞、資料集／領域、查詢日期與結果；同名詞跨領域時，依目標文件的工程語境選擇，不得只依第一筆結果。
- 機械、機器人與控制相關內容優先查「機械工程名詞」、「機械名詞－兩岸機械名詞」及「兩岸對照名詞－機械」。必要時使用網站的術語比對功能或下載資料集進行批次核對。
- 將每個候選詞分為「官方名詞一致」、「官方兩岸對照可修訂」、「資料庫未收錄」及「語意／文體判斷」。僅前兩類可列為高信心修訂；未收錄或只有字面差異的詞不可標示為錯誤。
- 樂詞網的兩岸對照表示對應關係，並非唯一可用寫法。除非該筆資料、上下文及全篇術語均支持，否則列為建議而不強制改寫。
- 檢閱報告逐項標示信心層級與依據類型（樂詞網兩岸對照／樂詞網領域名詞／未收錄的語意建議／保留），並附上樂詞網查詢連結或查詢詞。
